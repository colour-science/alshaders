...
enum DebugModes { kOff = 0, kLayer1, kLayer2, kMixer };
...
node_update {
    ...
    // set up AOVs
    REGISTER_AOVS
    data->standardAovs = AiNodeGetBool(node, "standardCompatibleAOVs");
...
shader_evaluate {
    ...
    if (debug == kMixer) {
        ...
        static AtString str_als_raytype("als_raytype");
        AiStateGetMsgInt(str_als_raytype, &als_raytype);
        static AtString str_als_context("als_context");
        static AtString str_als_deepGrouPtr("als_deepGroupPtr");
        ...
        if (als_raytype != ALS_RAY_SSS && mix >= (1.0f - IMPORTANCE_EPS)) {
            ...
            {
                AiStateSetMsgInt(str_als_context, ALS_CONTEXT_LAYER);
                // RGB AOVs
                AtRGB tmp[NUM_AOVs];
                ...
                AiStateSetMsgInt(str_als_context, ALS_CONTEXT_NONE);
            } else // just layer the results
            {
                ...
                AtRGB layer1 = AiShaderEvalParamRGB(p_layer1);
                // AtRGB layer1_opacity = sg->out_opacity;
                if (AiStateGetMsgPtr(str_als_deepGrouPtr,
                                     (void**)&deepGroupPtr)) {
