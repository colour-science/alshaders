...
const char* param_names[] = {
    "layer1", "layer1a", "layer1blend", "layer1name", "layer1enabled", "layer2",
    "layer2a", "layer2blend", "layer2name", "layer2enabled", "layer3",
    "layer3a", "layer3blend", "layer3name", "layer3enabled", "layer4",
    "layer4a", "layer4blend", "layer4name", "layer4enabled", "layer5",
    "layer5a", "layer5blend", "layer5name", "layer5enabled", "layer6",
    "layer6a", "layer6blend", "layer6name", "layer6enabled", "layer7",
    "layer7a", "layer7blend", "layer7name", "layer7enabled", "layer8",
    "layer8a", "layer8blend"
               "layer8name",
    "layer8enabled"};
...
const char* BlendModeNames[] = {
    ...
    "Color Burn", "Linear Dodge", "Linear Burn", "Linear Light", "Vivid Light",
    "Pin Light",  "Hard Mix",     "Reflect",     "Glow",         "Phoenix",
    NULL};
...
inline AtRGB overlay(const AtRGB& l1, const AtRGB& l2) {
    ...
    for (int i = 0; i < 3; ++i) {
        l1[i] < 0.5f
            ? result[i] = 2.0f * l1[i] * l2[i]
            : result[i] = (1.0f - 2.0f * (1.0f - l1[i]) * (1.0f - l2[i]));
    }
...
inline AtRGB softlight(const AtRGB& l1, const AtRGB& l2) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (l2[i] < 0.5f)
            result[i] =
                2.0f * l1[i] * l2[i] + AiSqr(l1[i]) * (1.0f - 2.0f * l2[i]);
        else
            result[i] = sqrtf(l1[i]) * (2.0f * l2[i] - 1.0f) +
                        2.0f * l1[i] * (1.0f - l2[i]);
    }
...
inline AtRGB linearlight(const AtRGB& l1, const AtRGB& l2, bool clamp_result) {
    ...
    if (clamp_result)
        result = clamp(result, AI_RGB_BLACK, AI_RGB_WHITE);
...
AtRGB blend(const AtRGB& l1, const AtRGB& l2, float a, int mode,
    ...
    case BM_LINEARDODGE:
        result = l1 + l2;
        if (clamp_result)
            result = min(result, AI_RGB_WHITE);
        break;
    case BM_SUBTRACT:
    case BM_LINEARBURN:
        result = l1 + l2 - AI_RGB_WHITE;
        if (clamp_result)
            result = max(result, AI_RGB_BLACK);
        break;
    case BM_DIFFERENCE:
        result = fabs(l1 - l2);
        break;
...
shader_evaluate {
    ...
    for (int i = 0; i <= data->max_layer; ++i) {
        ...
        if (layerEnabled) {
            result =
                blend(result, layerVal, layerAlpha, layerBlend, clamp_result);
        }
    }
