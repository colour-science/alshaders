...
enum FresnelModes { FM_DIELECTRIC = 0, FM_METALLIC };
...
node_update{
    ...
    delete[] data->perm_table_sss;
    data->perm_table_sss =
        new int[data->AA_samples * data->sss_bssrdf_samples2 *
                data->total_depth];
    ...
    if (data->aov_sss_clamp == 0.0f)
        data->aov_sss_clamp = AI_INFINITE;
    ...
    if (data->aov_emission_clamp == 0.0f)
        data->aov_emission_clamp = AI_INFINITE;
    ...
    if (data->transmissionCausticPaths)
        data->transmissionDoDirect = true;
    */
};
...
shader_evaluate {
    ...
    if (minh(ssAttenuationColor) < 1.0f) {
        ssAttenuationColor =
            clamp(ssAttenuationColor, AI_RGB_BLACK, rgb(0.999f));
    }
    ...
    if (minh(ssAttenuationColor) < 1 || ssInScatteringStrength > 0 ||
        ...
        if (ssSpecifyCoefficients) {
            ...
            // if we're doing a scattering medium, then make sure absorption is
            // not
            // 0
            if (ssInScatteringStrength > 0)
                sigma_a = max(sigma_a, rgb(0.01f));
            sigma_s =
                ssAttenuationColor * ssInScatteringStrength * ssDensityScale;
            sigma_t = sigma_s + sigma_a;
            if (maxh(sigma_a) > 0)
                do_attenuation = true;
            if (maxh(sigma_s) > 0)
    ...
    bool transmitAovs = data->transmitAovs;
    ...
    // If we're in a camera ray, try and allocate the array for transmitting the
    // AOVs, if transmitAovs is enabled
    ...
    } else // otherwise, we're in a secondary ray so turn on transmitAovs if it
           // has been turned on upstream
    {
    ...
    if (als_raytype == ALS_RAY_SSS) {
        ...
        alsIrradiateSample(
            sg, diffusion_msgdata, data->diffuse_sampler, U, V,
            data->lightGroups, path_throughput, data->trace_set_sss.c_str(),
            ...
            if (kt >= IMPORTANCE_EPS) // else surface is fully reflective
            {
                if (maxh(sigma_t) > 0.0f) {
                    ...
                    if (AiStateGetMsgPnt("alsPreviousIntersection",
                        ...
                        // 'add' the extra opacity due to transmission colour
                        // and
                        // fresnel
                        outOpacity = 1.0f - ((1.0f - outOpacity) * kt *
                                             transmissionColor);
                    } else {
                        // first intersection
                        // tell the next shader invocation that we're now inside
                        // the
    ...
    // do cryptomattes before early out for fully transparent but after opacity
    // is set
    ...
    if (data->specular1NormalConnected) {
        specular1Normal =
            AiV3Normalize(AiShaderEvalParamVec(p_specular1Normal));
    }

    AtVector specular2Normal = sg->Nf;
    if (data->specular2NormalConnected) {
        specular2Normal =
            AiV3Normalize(AiShaderEvalParamVec(p_specular2Normal));
    }
    ...
    if (sg->Rr > 0) {
        ...
        if (data->do_rr) {
            roughness_x =
                std::max(roughness_x,
                         alsPreviousRoughness * specular1RoughnessDepthScale);
            roughness_y =
                std::max(roughness_y,
                         alsPreviousRoughness * specular1RoughnessDepthScale);
            roughness2_x =
                std::max(roughness2_x,
                         alsPreviousRoughness * specular2RoughnessDepthScale);
            roughness2_y =
                std::max(roughness2_y,
                         alsPreviousRoughness * specular2RoughnessDepthScale);
            transmissionRoughness = std::max(
    ...
    // Set up flags to early out of calculations based on where we are in the
    // ray
    // tree
    ...
    // if the user has chosen not to do internal direct reflections, turn off
    // the
    ...
    sg->Nf =
        diffuseNormal; // or should that be: sg->N = sg->Nf = diffuseNormal?
    ...
    if (do_glossy_direct) {
        ...
        while (AiLightsGetSample(sg)) {
            if (AiLightGetAffectSpecular(sg->Lp)) {
                ...
                brdfw.ibs = false;
                LspecularDirect = AiEvaluateLightSample(
                                      sg, &brdfw, GlossyMISSample_wrap,
    ...
    if (do_glossy2_direct) {
        ...
        while (AiLightsGetSample(sg)) {
            if (AiLightGetAffectSpecular(sg->Lp)) {
                ...
                brdfw2.ibs = false;
                Lspecular2Direct = AiEvaluateLightSample(
                                       sg, &brdfw2, GlossyMISSample_wrap,
                                       GlossyMISBRDF_wrap, GlossyMISPDF_wrap) *
        ...
        if (brdfw2.ns > 0.0f)
            kti *=
                1.0f - (maxh(brdfw2.kr_int) / brdfw2.ns * maxh(specular2Color));
    }
    ...
    if (do_diffuse) {
        ...
        while (AiLightsGetSample(sg)) {
            if (AiLightGetAffectDiffuse(sg->Lp)) {
                ...
                LdiffuseDirect = AiEvaluateLightSample(
                                     sg, dmis, AiOrenNayarMISSample,
                                     AiOrenNayarMISBRDF, AiOrenNayarMISPDF) *
                                 diffuse_strength * kti * (1 - sssMix);
                if (lightGroup >= 0 && lightGroup < NUM_LIGHT_GROUPS) {
                    lightGroupsDirect[lightGroup] +=
                        LdiffuseDirect * diffuseColor;
                    shadowGroups[lightGroup].rgb() +=
                        sg->Liu * sg->we *
                        std::max(0.0f, AiV3Dot(sg->Nf, sg->Ld)) * sg->Lo * kti *
                        diffuseColor * AI_ONEOVERPI;
    ...
    if (do_transmission && data->transmissionDoDirect &&
        ...
        while (AiLightsGetSample(sg)) {
            if (AiLightGetAffectSpecular(sg->Lp)) {
                ...
                LtransmissionDirect =
                    AiEvaluateLightSample(sg, mft,
                                          MicrofacetTransmission::Sample,
    ...
    if (do_glossy && specular1IndirectStrength > 0.0f) {
        ...
        if (roughness == 0.0f) {
            if (!tir) {
                ...
                if (!rr_transmission) {
                    kr = brdfw.fr->kr(
                        std::max(0.0f, AiV3Dot(wi_ray.dir, sg->Nf)));
                    kti = maxh(kr);
                } else {
                    kr = 1.0f;
                    kti = 1.0f;
                }
                // Previously we pulled the sampler here as an optimization.
                // This
                // nets us about a 10-30%
                // speedup in the case of pure dielectrics, but severely fucks
                // up
                // sss, both on the surface
                // being cast, and in reflected surfaces.
                // Remove this for now until we can figure out exactly what's
                // going
                // on
                ...
                if (cont) {
                    ...
                    if (maxh(kr) > IMPORTANCE_EPS) {
                        ...
                        result_glossyIndirect = min(
                            scrs.color * f, rgb(data->specular1IndirectClamp));
                        if (doDeepGroups && hit) {
                ...
                sg->Nf = Nfold;
                kti = 1.0f -
                      kti * maxh(specular1Color) * specular1IndirectStrength;
            }
        } else {
            AiMakeRay(&wi_ray, AI_RAY_GLOSSY, &sg->P, NULL, AI_BIG, sg);
            kti = 0.0f;
            ...
            while (AiSamplerGetSample(sampit, samples)) {
                ...
                if (AiV3Dot(wi, specular1Normal) > 0.0f) {
                    ...
                    kti += maxh(kr);
                    if (maxh(kr) > IMPORTANCE_EPS) // only trace a ray if it's
                                                   // going to matter
                    {
                        ...
                        float rr_p = 1.0f;
#ifdef RR_BOUNCES
                        rr_p = std::min(1.0f, sqrtf(maxh(throughput) /
                                                    maxh(path_throughput)));
                        if (data->do_rr && sg->Rr > 0) {
                            cont = false;
                            // get a permuted, stratified random number
                            int pb = data->AA_samples * data->glossy_samples2;
...
#endif
                        AiStateSetMsgRGB("als_throughput", throughput);
                        // if we're in a camera ray, pass the sample index down
                        // to the
                        // child SG
                        if (cont) {
                            bool hit = AiTrace(&wi_ray, &scrs);
                            ...
                            // accumulate the lightgroup contributions
                            // calculated by
                            // the child shader
                            if (doDeepGroups && hit) {
                                for (int i = 0; i < NUM_LIGHT_GROUPS; ++i) {
    ...
    if (do_glossy2) {
        ...
        AtSamplerIterator* sampit =
            AiSamplerIterator(data->glossy2_sampler, sg);
        AiMakeRay(&wi_ray, AI_RAY_GLOSSY, &sg->P, NULL, AI_BIG, sg);
        kti2 = 0.0f;
        AtRGB kr;
        AiStateSetMsgFlt("alsPreviousRoughness",
            ...
            if (AiV3Dot(wi, specular2Normal) > 0.0f) {
                ...
                if (data->do_rr && sg->Rr > 0) {
                    ...
                    float u = (float(data->perm_table_spec2[idx]) +
                               sampleTEAFloat(
                                   idx, TEA_STREAM_ALSURFACE_RR_SPEC2_JITTER)) /
                              float(pb);
                    // offset based on pixel
                    float offset =
                        sampleTEAFloat(sg->y * data->xres + sg->x,
...
#endif
                if (cont && maxh(kr) > IMPORTANCE_EPS) // only trace a ray if
    ...
    if (do_diffuse &&
        ...
        float kr = kti * kti2;
        AtSamplerIterator* sampit =
            AiSamplerIterator(data->diffuse_sampler, sg);
        AiMakeRay(&wi_ray, AI_RAY_DIFFUSE, &sg->P, NULL, AI_BIG, sg);
        int ssi = 0;
        while (AiSamplerGetSample(sampit, samples)) {
            // cosine hemisphere sampling as O-N sampling does not work outside
            // of
            // a light loop
        ...
        if (doDeepGroups) {
            for (int i = 0; i < NUM_LIGHT_GROUPS; ++i) {
                deepGroupsDiffuse[i] *= invns * diffuseColor *
                                        diffuseIndirectStrength * (1 - sssMix);
            }
        }
    ...
    if (do_transmission) {
        ...
        if (transmissionRoughness == 0.0f) {
            ...
            if (refraction) {
                // commented this sampler pull out to match the spec. This will
                // make
                ...
                if (sg->Rr_refr < data->GI_refraction_depth) {
                    if (kti * kti2 > IMPORTANCE_EPS) {
                        ...
                        if (maxh(sigma_t) > 0.0f && !inside) {
                            transmittance.r =
                                fast_exp(float(-sample.z) * sigma_t.r);
                            transmittance.g =
                                fast_exp(float(-sample.z) * sigma_t.g);
                            transmittance.b =
                                fast_exp(float(-sample.z) * sigma_t.b);
                        }
                        ...
                        // single scattering
                        if (do_attenuation && maxh(sigma_s_prime) > 0.0f &&
                            !inside && do_scattering) {
                            result_ss += AiSSSTraceSingleScatter(
                                sg, bssrdfbrdf(sigma_s_prime / sigma_t_prime),
                                mfp, ssDirection, transmissionIor);
                        }
            ...
            } else // total internal reflection
            {
                if (sg->Rr_refr < data->GI_refraction_depth) {
                    ...
                    result_transmission += min(sample.color * transmittance,
                                               rgb(data->transmissionClamp));
                    assert(AiIsFinite(result_transmission));
                    // accumulate the lightgroup contributions calculated by the
                    // child
                    ...
                    if (transmitAovs) {
                        for (int i = 0; i < NUM_AOVs; ++i) {
                            childAovs[i] +=
                                transmittedAovPtr[i] * transmittance;
                        }
                    }
                } else // trace the background if we've hit the refraction depth
                // limit
            ...
            MicrofacetTransmission* mft = MicrofacetTransmission::create(
                sg, transmissionRoughness, transmissionRoughness, t_eta, sg->Nf,
                U, V);

            while (AiSamplerGetSample(sampit, samples)) {
                AtVector m =
                    mft->sampleMicrofacetNormal(samples[0], samples[1]);
    ...
    if (do_sss) {
        if (data->sssMode == SSSMODE_CUBIC) {
            ...
            AtRGB radius = max(rgb(0.0001), sssRadius * sssRadiusColor /
                                                (sssDensityScale * 7));
            ...
            if (sssWeight2 == 0.0f && sssWeight3 == 0.0f) {
                ...
                AtRGB r2 = max(rgb(0.0001), sssRadius2 * sssRadiusColor2 /
                                                (sssDensityScale * 7));
                AtRGB r3 = max(rgb(0.0001), sssRadius3 * sssRadiusColor3 /
                                                (sssDensityScale * 7));
                AtRGB weights[9] = {
                    AI_RGB_RED * sssWeight1,   AI_RGB_GREEN * sssWeight1,
                    AI_RGB_BLUE * sssWeight1,  AI_RGB_RED * sssWeight2,
                ...
                AiBSSRDFEmpirical(sg, result_sss_direct, result_sss_indirect, r,
                                  &c[0].r, weights, 9);
                result_sss = result_sss_direct + result_sss_indirect;
            }
        } else {
            ...
            result_sss = alsDiffusion(
                ...
                result_sss_indirect, lightGroupsSss, deepGroupsSss,
                deepGroupPtr, data->trace_set_sss.c_str(),
    ...
    if (sg->Rt & AI_RAY_CAMERA) {
        ...
        if (data->standardAovs) {
            ...
            if (result_emission != AI_RGB_BLACK)
                AiAOVSetRGB(sg, data->aovs[k_emission].c_str(),
                            result_emission);
            ...
            for (int i = 0; i < NUM_AOVs; ++i) {
                if (i == k_refraction) {
                    ...
                    AtRGB tmp = result_glossyDirect +
                                transmittedAovPtr[k_direct_specular];
                    if (tmp != AI_RGB_BLACK) {
                ...
                if (transmittedAovPtr[i] != AI_RGB_BLACK)
                    AiAOVSetRGB(sg, data->aovs[i].c_str(),
                                transmittedAovPtr[i]);
            }
            if (doDeepGroups) {
                ...
                for (int i = 0; i < NUM_LIGHT_GROUPS; ++i) {
                    ...
                    if (deepGroups[i] != AI_RGB_BLACK) {
                        deepGroups[i] = min(
                            deepGroups[i], rgb(data->aov_light_group_clamp[i]));
            ...
            for (int i = 0; i < NUM_LIGHT_GROUPS; ++i) {
                if (shadowGroups[i] != AI_RGBA_BLACK)
                    AiAOVSetRGBA(sg,
                                 data->aovs_rgba[k_shadow_group_1 + i].c_str(),
                                 shadowGroups[i]);
            }

            diffuseColor =
                min(diffuseColor, rgb(data->aov_diffuse_color_clamp));
            result_diffuseDirect =
                min(result_diffuseDirect, rgb(data->aov_direct_diffuse_clamp));
            result_diffuseDirectRaw =
                min(result_diffuseDirectRaw,
                    rgb(data->aov_direct_diffuse_raw_clamp));
            result_backlightDirect = min(result_backlightDirect,
                                         rgb(data->aov_direct_backlight_clamp));
            result_sss = min(result_sss, rgb(data->aov_sss_clamp));
            result_glossyDirect =
                min(result_glossyDirect, rgb(data->aov_direct_specular_clamp));
            result_glossy2Direct = min(result_glossy2Direct,
                                       rgb(data->aov_direct_specular_2_clamp));
            result_diffuseIndirect = min(result_diffuseIndirect,
                                         rgb(data->aov_indirect_diffuse_clamp));
            result_diffuseIndirectRaw =
                min(result_diffuseIndirectRaw,
                    rgb(data->aov_indirect_diffuse_raw_clamp));
            result_backlightIndirect =
                min(result_backlightIndirect,
                    rgb(data->aov_indirect_backlight_clamp));
            result_glossyIndirect = min(result_glossyIndirect,
                                        rgb(data->aov_indirect_specular_clamp));
            result_glossy2Indirect =
                min(result_glossy2Indirect,
            ...
            result_emission =
                min(result_emission, rgb(data->aov_emission_clamp));

            if (diffuseColor != AI_RGB_BLACK)
                AiAOVSetRGB(sg, data->aovs[k_diffuse_color].c_str(),
                            diffuseColor);
            ...
            if (result_ss != AI_RGB_BLACK)
                AiAOVSetRGB(sg, data->aovs[k_single_scatter].c_str(),
                            result_ss);
            if (result_transmission != AI_RGB_BLACK)
                AiAOVSetRGB(sg, data->aovs[k_refraction].c_str(),
                            result_transmission);
    ...
    } else // we're in a secondary ray //
    {
        ...
        if (transmitAovs) {
            if (do_transmission) {
                ...
                transmittedAovPtr[k_direct_diffuse_raw] =
                    result_diffuseDirectRaw;
                ...
                transmittedAovPtr[k_indirect_backlight] =
                    result_backlightIndirect;
                ...
                transmittedAovPtr[k_indirect_specular_2] =
                    result_glossy2Indirect;
                ...
                for (int i = 0; i < NUM_LIGHT_GROUPS; ++i) {
                    transmittedAovPtr[k_light_group_1 + i] =
                        lightGroupsDirect[i];
                }
                for (int i = 0; i < NUM_ID_AOVS; ++i) {
