...
ColorSystem CsNTSC("NTSC", point2(0.67, 0.33), point2(0.21, 0.71),
                   point2(0.14, 0.08), illumC);
ColorSystem CsSMPTE("SMPTE", point2(0.630, 0.340), point2(0.31, 0.595),
                    point2(0.155, 0.07), illumD65);
ColorSystem CsHDTV("HDTV", point2(0.670, 0.330), point2(0.21, 0.71),
                   point2(0.150, 0.06), illumD65);
ColorSystem CsCIE("CIE", point2(0.7355, 0.2645), point2(0.2658, 0.7243),
                  point2(0.1669, 0.085), illumE);
ColorSystem CsRec709("Rec709", point2(0.64, 0.33), point2(0.30, 0.60),
                     point2(0.15, 0.06), illumD65);

float cieMatch[81][3] = {{0.0014, 0.0000, 0.0065}, {0.0022, 0.0001, 0.0105},
...
float cineonToLin(float c) {
    ...
    static const float Gain =
        1.0f / (1.0f - powf(10.0f, (RefBlack - RefWhite) * 0.002 / 0.6));
...
float logCToLin(float c) {
    if (c > 0.1496582f)
        return (powf(10.0f, (c - 0.385537f) / 0.2471896f) - 0.052272f) /
               5.555556f;
    else
        return (c - 0.092809f) / 5.367655f;
}
