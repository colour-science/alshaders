...
#define REMAP_FLOAT_PARAM_ENUM                                                 \
    p_RMPinputMin, p_RMPinputMax, p_RMPcontrast, p_RMPcontrastPivot,           \
        p_RMPbiasVal, p_RMPgainVal, p_RMPoutputMin, p_RMPoutputMax,            \
        p_RMPclampEnable, p_RMPthreshold, p_RMPclampMin, p_RMPclampMax
...
struct RemapFloat {
public:
    RemapFloat(float imn, float imx, float ct, float ctp, float bs, float gn,
               float omn, float omx, bool ce, bool t, float cmn, float cmx)
        : inputMin(imn), inputMax(imx), contrastVal(ct), contrastPivot(ctp),
          bias(bs), gain(gn), outputMin(omn), outputMax(omx), clampEnable(ce),
          threshold(t), clampMin(cmn), clampMax(cmx) {}
...
#define REMAP_FLOAT_CREATE                                                     \
    RemapFloat(AiShaderEvalParamFlt(p_RMPinputMin),                            \
